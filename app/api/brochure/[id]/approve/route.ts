...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (session.role !== "admin") {
    return NextResponse.json({ error: "Only admin can approve brochures." }, { status: 403 });
  }
  ...
  try {
    ...
    const brochure = await decideBrochure(brochureId, session, "approved", body.content, null);
    ...
    return NextResponse.json({ error: "Failed to approve brochure." }, { status: 500 });
  }
}
