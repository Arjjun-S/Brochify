...
const KonvaEngine = dynamic(() => import('@/components/Editor/KonvaEngine'), {
  ssr: false,
});
...
export default function Dashboard() {
  ...
  const handleEdit = (path: string, newValue: any) => {
    setBrochureData((prevData: any) => {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
            ...
            current[keys[i]] = { ...current[keys[i]] }; // Create a new object reference
            current = current[keys[i]];
        }
        
        // Only update if the value actually changed to prevent unnecessary re-renders
        if (current[keys[keys.length - 1]] !== newValue) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Find all canvas elements generated by Konva
      const canvases = document.querySelectorAll('.konva-container-export canvas');
      if (canvases.length === 0) {
          throw new Error("No canvas elements found to export");
      }

      let pagesHtml = '';
      canvases.forEach(canvas => {
          const imgData = (canvas as HTMLCanvasElement).toDataURL('image/png', 2.0); // High quality
          pagesHtml += `
            <div class="brochure-page" style="page-break-after: always; width: 260mm; height: 180mm; margin: 0; padding: 0;">
    ...
    <main className="h-screen bg-[#F8FAFC] flex flex-col font-sans overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="flex-1 bg-[#F0F4F8] p-4 overflow-y-auto h-full flex flex-col items-center relative group scroll-smooth">
            ...
            {brochureData ? (
                <div className="w-full flex flex-col items-center gap-10 py-6 relative z-10" id="brochure-preview">
                    <KonvaEngine initialData={brochureData} selectedLogos={selectedLogos} />
                </div>
            ) : (
