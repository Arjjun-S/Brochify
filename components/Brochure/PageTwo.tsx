...
export default function PageTwo({ data, selectedLogos, onEdit }: PageTwoProps) {
  return (
    <div id="brochure-page-2" className="brochure-page border border-gray-200" style={{ backgroundColor: '#ffffff' }}>
      ...
      {/* Column 1: About Sections (Blue) */}
      <div className="column column-blue text-[11px] flex flex-col gap-6 !p-6" style={{ backgroundColor: '#0047AB', color: '#ffffff' }}>
        <div className="text-center">
            <h3 className="text-base font-black mb-3 uppercase tracking-tighter inline-block px-4 py-1 bg-white/10 rounded-full" style={{ borderBottom: '1px solid rgba(255,255,255,0.3)' }}>About SRM</h3>
            <p className="leading-tight text-justify opacity-90">{data.aboutCollege}</p>
        </div>
        <div className="text-center">
            <h3 className="text-[13px] font-black mb-3 uppercase tracking-tighter inline-block px-4 py-1 bg-white/10 rounded-full" style={{ borderBottom: '1px solid rgba(255,255,255,0.3)' }}>About the School</h3>
            <p className="leading-tight text-justify opacity-90">{data.aboutSchool}</p>
        </div>
      </div>

      {/* Column 2: About Dept & FDP (White) */}
      <div className="column column-white text-[11px] border-x flex flex-col gap-6 !p-6" style={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0' }}>
         <div className="text-center">
            <h3 className="text-[13px] font-black mb-3 uppercase tracking-tighter inline-block px-4 py-1 rounded-full" style={{ color: '#0047AB', backgroundColor: '#0047AB1a', borderBottom: '1px solid rgba(0,71,171,0.2)' }}>About Department</h3>
            <p className="leading-tight text-justify" style={{ color: '#334155' }}>{data.aboutDepartment}</p>
        </div>
        <div className="text-center">
            <h3 className="text-[13px] font-black mb-3 uppercase tracking-tighter inline-block px-4 py-1 rounded-full" style={{ color: '#0047AB', backgroundColor: '#0047AB1a', borderBottom: '1px solid rgba(0,71,171,0.2)' }}>About the FDP</h3>
        ...
        <div className="grid grid-cols-2 gap-2 flex-1">
            {data.speakers?.slice(0, 8).map((s: any, i: number) => (
                <div key={i} className="p-1.5 rounded border flex flex-col justify-center" style={{ backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.1)' }}>
                    <p className="font-black text-[9.5px] leading-none mb-0.5" style={{ color: '#fde047' }}>{s.name}</p>
                    <p className="text-[8.5px] italic leading-tight truncate" style={{ color: 'rgba(255,255,255,0.7)' }}>{s.role}</p>
                    <p className="text-[8px] leading-tight truncate" style={{ color: 'rgba(255,255,255,0.5)' }}>{s.org}</p>
                </div>
            ))}
        </div>
