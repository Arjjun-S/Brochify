...
export default function PageOne({ data, selectedLogos, onEdit }: PageOneProps) {
  ...
  const convener = data.committee?.find((c: any) => c.role?.toLowerCase().includes('convener') && !c.role?.toLowerCase().includes('co-'));
  const coConvener = data.committee?.find((c: any) => c.role?.toLowerCase().includes('co-convener'));
  const advisory = data.committee?.filter((c: any) => c.role?.toLowerCase().includes('advisory')) || [];
  const organizing = data.committee?.filter((c: any) => 
    !c.role?.toLowerCase().includes('patron') && 
    !c.role?.toLowerCase().includes('advisory') &&
  ...
  return (
    <div id="brochure-page-1" className="brochure-page border border-gray-200" style={{ backgroundColor: '#ffffff' }}>
                ...
                <div className="flex gap-4">
                    {convener && (
                        <div className="flex-1">
                            ...
                            <p className="text-[11px] leading-tight font-bold" style={{ color: '#1e293b' }}>{convener.name}</p>
                            <p className="text-[10px] leading-tight opacity-70" style={{ color: '#1e293b' }}>{convener.role}</p>
                        </div>
                    )}
                    {coConvener && (
                        <div className="flex-1">
                            <h4 className="text-[#0047AB] text-[11px] font-black uppercase tracking-wider border-b border-[#0047AB]/20 mb-1">CO-CONVENER</h4>
                            <p className="text-[11px] leading-tight font-bold" style={{ color: '#1e293b' }}>{coConvener.name}</p>
                            <p className="text-[10px] leading-tight opacity-70" style={{ color: '#1e293b' }}>{coConvener.role}</p>
                        </div>
                    )}
                </div>
            )}
      ...
      <div className="column column-blue flex flex-col items-center !p-4" style={{ backgroundColor: '#0047AB', color: '#ffffff' }}>
        ...
        <p className="text-[9px] mb-4 break-all max-w-[120px] text-center" style={{ color: 'rgba(255,255,255,0.6)' }}>{data.googleForm}</p>
